...
internal class AndroidLua: Lua {
    ...
    override fun register(
        ...
        if (name != null && function != null) {
            // Wrap the common LuaFunction to the platform-specific one
            val wrappedFunction = party.iroiro.luajava.value.LuaFunction { lua, args ->
                val commonArgs = args.map { it as? LuaValue }
                val result = function.call(lua as Lua?, commonArgs)
                // Convert back to platform types
                result as Array<out party.iroiro.luajava.value.LuaValue>?
            }
            lua.register(name, wrappedFunction)
        }
    }

    override fun eval(command: String?): Array<LuaValue?>? {
        return command?.let {
            lua.eval(it) as Array<party.iroiro.luajava.value.LuaValue?> as Array<LuaValue?>
        }
    }
    ...
    override fun from(buffer: ByteArray?): LuaValue? {
        return buffer?.let { lua.from(ByteBuffer.wrap(it)) as? party.iroiro.luajava.value.LuaValue as LuaValue }
    }
}
