 ...
 *
 * This class uses the interpreter.py module to execute arbitrary Python code
 * in a separate thread with timeout handling.
 */
object PythonExecutor {
    ...
    fun <T> executeAndParse(code: String, parser: (String) -> T): Result<T> {
        return executeSafe(code).mapCatching { output ->
            parser(output.trim())
        }
    }
}
