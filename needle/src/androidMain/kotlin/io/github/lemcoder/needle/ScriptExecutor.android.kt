...
actual fun createExecutor(context: Any): Executor {
    context as Context

    val engine = instantiateScriptEngine()
    val logModule = LuaLoggingModule(engine)
    val networkModule = LuaNetworkModule(engine)
    val fileSystemModule = LuaFileSystemModule(engine, context.filesDir)
